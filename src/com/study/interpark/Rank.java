...
public class Rank {
   ...
   * [자료구조]
   *
   * 1] 우선순위 큐 - 학생의 인덱스와 점수를 저장한 자료구조를 저장 (제거 순은 점수가 높은 순)
   * 2] 맵 -  점수별로 동점자수가 몇 명인지 저장
   *
   * [로직]
   * (자료구조 구성)
   * 1] input 을 바탕으로 scoreQueue 와 scoreCountMap 을 구성
   *
   * (result 배열 구성)
   * 2] queue 가 empty 될때까지 poll 수행
   * 2-1] peek 한 것의 score 를 참조해서 map 에서 동점자 수를 가지고 온다.
   * 2-2] 가지오 온 동점자 수 만큼 poll 하고 등수를 반영한다.
   */
